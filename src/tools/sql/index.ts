...
//     meta,
//     registry,
//     // Optional controls for tool-level RBAC & discovery tools visibility
//     tools, // { schema?: boolean; peek?: boolean; query?: boolean }
//     dataPolicy,
//     userContext,
//     discoveryVisible, // boolean; if false, do NOT register discovery tools
...
//           title: "List databases aliases",
//           description:
//             "Return the list of available database aliases on this server (e.g., hr, finance, library).",
//           // ✅ MCP SDK expects ZodRawShape; empty shape means “no args required”
//           inputSchema: {},
//         },
//         async () => {
//           try {
...
//           title: "List available database (types)",
//           description: "List available database dialects (types) visible in this session.",
//           // ✅ empty raw shape (no args required)
//           inputSchema: {},
//         },
//         async () => {
//           try {
...
//           title: "List database names",
//           description:
//             "List database names (not aliases) visible in this session (unique, sorted).",
//           // ✅ empty raw shape (no args required)
//           inputSchema: {},
//         },
//         async () => {
...
//       // db.listByType (already uses raw shape; kept unchanged)
//       server.registerTool(
//         "db.listByType",
//         {
//           title: "List databases by type",
...
//     const sep = `${headers.map(() => "---").join(" | ")}\n`;
//     const body = rows
//       .map((r) => `${headers.map((h) => fmt(r[h])).join(" | ")}`)
//       .join("\n");
//     return [top, sep, body].join("");
//   }
...
//     return String(v);
//   }
//   function quoteIdent(dialect: DB["dialect"], ident: string): string {
//     switch (dialect) {
//       case "pg":
//       case "oracle":
//       case "sqlite": {
...
//     }
//   }
//   function quoteMaybeQualified(dialect: DB["dialect"], ident: string): string {
//     if (ident.includes(".")) {
//       const [schema, name] = ident.split(".");
//       return `${quoteIdent(dialect, schema)}.${quoteIdent(dialect, name)}`;
//     }
...
//             const filter = dataPolicy?.rowFilters?.[bare];
//             if (filter) {
//               if (/:\s*user_id\b/.test(filter) && userContext?.user_id === undefined) {
//                 throw new Error("Missing user identity (user_id) for row-level policy.");
//               }
//               effectiveSql = addWhere(effectiveSql, filter);
//               if (userContext?.user_id !== undefined) {
...
//         await audit(
//           `[${new Date().toISOString()}] ${db.dialect} rows=${rowCount ?? limited?.length ?? 0} ms=${ms} sql=${effectiveSql}`
//         );
...
//       // db.aliases (no inputSchema -> accept null/omitted args)
//       server.registerTool(
//         "db.aliases",
//         {
//           title: "List databases aliases",
...
//       // db.listByType (has inputs -> keep raw shape)
//       server.registerTool(
//         "db.listByType",
//         {
//           title: "List databases by type",
...
//             includeAliases ? { alias: i.alias, name: i.databaseName } : { name: i.databaseName }
//           );
//           return { content: [{ type: "text", text: JSON.stringify(rows, null, 2) }] };
//         }
//       );
...
//       },
//       async ({ sql }) => {
//         return { content: [{ type: "text", text: `_query logic for ${sql}_` }] };
//       }
//     );
//   }
// }
...
//           // Normalize arguments: if it's a string, parse it
...
//             }
//           }
//           // Now args is guaranteed to be an object
//           const aliases = Array.from(serverAliases.get(server) ?? new Set<string>()).sort();
//           return { content: [{ type: "text", text: JSON.stringify(aliases, null, 2) }] };
...
// // ───────────────────────────────────────────────────────────────────────────────
// // NEW: helpers to bypass SDK pre‑validation (which rejects string "{}")
// // ───────────────────────────────────────────────────────────────────────────────
...
// /**
//  * Register a tool WITHOUT inputSchema so the SDK doesn't pre‑validate.
//  * Then normalize (string→object) and optionally validate with a Zod shape at runtime.
//  */
// function registerToolNoSchema<
//   TShape extends z.ZodRawShape | null,
//   TParsed = TShape extends z.ZodRawShape ? z.infer<z.ZodObject<TShape>> : any
// >(
//   server: McpServer,
//   name: string,
//   meta: { title?: string; description?: string },
//   shape: TShape,
//   handler: (args: TParsed) => Promise<any>
// ) {
//   server.registerTool(
//     name,
//     {
//       // IMPORTANT: no inputSchema here on purpose
//       title: meta.title,
...
//       const parsed =
//         shape ? (z.object(shape).parse(raw) as TParsed) : (raw as TParsed);
...
//     // db.aliases — NO inputSchema; normalize in handler
...
//       {
//         title: "List database names",
//         description: "List unique database names (not aliases) visible in this session.",
//       },
//       null,
//       async () => {
//         const visible = metaVisible();
...
//     // db.listByType — runtime validation with shape
//     const LIST_BY_TYPE = {
//       type: z.string().min(1).describe("Dialect: mysql\npg\nmssql\noracle\nsqlite"),
...
//           .filter(([alias, _m]) => allowed.has(alias))
//           .map(([, m]) => m)
//           .filter((m) => m.dialect === dialect);
...
// /* ────────────────────────────────────────────────────────────────────────────
//    Helpers to avoid SDK pre-validation issues (e.g., "arguments": "{}" string)
//    ──────────────────────────────────────────────────────────────────────────── */
// function normalizeArgsRaw(argsRaw: unknown): any {
//   if (typeof argsRaw === "string") {
//     try {
...
// /** Register a tool WITHOUT inputSchema so the SDK doesn’t pre‑validate.
//  *  We normalize arguments and (optionally) validate with a Zod shape at runtime.
//  */
// function registerToolNoSchema<
//   TShape extends z.ZodRawShape | null,
//   TParsed = TShape extends z.ZodRawShape ? z.infer<z.ZodObject<TShape>> : any
...
//       title: meta.title,
//       description: meta.description,
//       // NOTE: no inputSchema here on purpose
//     },
//     async (argsRaw) => {
//       const raw = normalizeArgsRaw(argsRaw);
//       const parsed =
...
// /* ────────────────────────────────────────────────────────────────────────────
//    Per-server state (unchanged idea)
//    ──────────────────────────────────────────────────────────────────────────── */
// const serverAliases = new WeakMap<McpServer, Set<string>>();
// const discoveryRegistered = new WeakSet<McpServer>();
...
//       // db.listByType  (args validated at runtime)
//       const LIST_BY_TYPE = {
//         type: z.string().min(1).describe("Dialect: mysql\npg\nmssql\noracle\nsqlite"),
//         unique: z.boolean().optional().default(true),
//         includeAliases: z.boolean().optional().default(false),
...
//   /* ────────────────────────────────────────────────────────────────────────
//      Namespaced tools (use runtime validation to avoid pre‑validation bug)
//      ──────────────────────────────────────────────────────────────────────── */
...
// function detectBaseTable(sql: string): string | null {
//   // Normalize whitespace and extract the first identifier after FROM
...
// /* ────────────────────────────────────────────────────────────────────────────
//    Helper functions (dialect-aware + bug fixes)
//    ──────────────────────────────────────────────────────────────────────────── */
// function toMarkdown(rows: any[]) {
//   if (!rows?.length) return "_(no rows)_";
//   const headers = Object.keys(rows[0]);
...
//     switch (dbX.dialect) {
//       case "pg": {
//         // Use positional parameter to avoid ':n' in text
//         sql = `SELECT * FROM ${qTable} LIMIT $1`;
//         params = [maxRows];
//         break;
//       }
//       case "mysql":
//       case "sqlite": {
//         // Use '?' placeholder for MySQL/SQLite
//         sql = `SELECT * FROM ${qTable} LIMIT ?`;
//         params = [maxRows];
//         break;
//       }
//       case "mssql": {
...
// /* ────────────────────────────────────────────────────────────────────────────
//    Helper functions (fixed to use dbX consistently + dialect-aware LIMIT)
//    ──────────────────────────────────────────────────────────────────────────── */
// function toMarkdown(rows: any[]) {
//   if (!rows?.length) return "_(no rows)_";
//   const headers = Object.keys(rows[0]);
...
//       title: meta.title,
//       description: meta.description,
//       // 👇 ADD: expose minimal inputSchema so strict clients are happy
//       inputSchema: (shape ?? EMPTY_SHAPE) as z.ZodRawShape,
//     },
//     async (argsRaw) => {
//       // Keep your manual parsing exactly as-is
//       const raw = normalizeArgsRaw(argsRaw);
//       const parsed =
//         shape ? (z.object(shape).parse(raw) as TParsed) : (raw as TParsed);
//       return handler(parsed);
...
//             content: [
//               { type: "json", json: dump },
//               { type: "text", text: JSON.stringify(dump, null, 2) },
//             ],
//           };
//         }
...
//             const filter = dataPolicy?.rowFilters?.[bare];
//             if (filter) {
//               // detect :user_id placeholder properly
//               if (/:user_id\b/.test(filter) && !userContext?.user_id) {
//                 throw new Error("Missing user identity (user_id) for row-level policy.");
//               }
//               effectiveSql = addWhere(effectiveSql, filter);
...
//         const ms = Date.now() - t0;
//         const limited =
//           Array.isArray(rows) && rows.length > rowLimit ? rows.slice(0, rowLimit) : rows;

//         await audit(
//           `[${new Date().toISOString()}] ${db.dialect} rows=${
//             rowCount ?? limited?.length ?? 0
//           } ms=${ms} sql=${effectiveSql}`
//         );
...
// // Empty raw shape for no-arg tools
// const EMPTY_SHAPE: z.ZodRawShape = {};
...
//       title: meta.title,
//       description: meta.description,
//       // ✅ Publish RAW SHAPE; strict MCP clients expect this
//       inputSchema: (shape ?? EMPTY_SHAPE) as z.ZodRawShape,
//     },
//     async (argsRaw) => {
//       // keep your manual parsing exactly as-is
//       const raw = normalizeArgsRaw(argsRaw);
//       const parsed = shape
//         ? (z.object(shape).parse(raw) as TParsed)    // local validation
//         : (raw as TParsed);
//       return handler(parsed);
//     }
//   );
...
//             const filter = dataPolicy?.rowFilters?.[bare];
//             if (filter) {
//               // FIX: detect :user_id placeholder properly
//               if (/:user_id\b/.test(filter) && !userContext?.user_id) {
//                 throw new Error("Missing user identity (user_id) for row-level policy.");
//               }
//               effectiveSql = addWhere(effectiveSql, filter);
