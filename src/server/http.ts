...
//   // --- Shim: normalize tool definitions & outputs for strict MCP clients (Azure Agents) ---
//   // Adds a default inputSchema {} and converts text-JSON outputs into { type: "json" }.
//   function patchRegisterTool(server: McpServer) {
//     const orig = (server as any).registerTool?.bind(server);
//     if (!orig) return;

//     (server as any).registerTool = (name: string, def: any, handler: any) => {
//       // Ensure inputSchema is a raw shape (empty object for no-arg tools)
//       const fixedDef = { ...def };
//       if (fixedDef.inputSchema == null) {
//         fixedDef.inputSchema = {}; // ZodRawShape: ok to publish {}
//       }

//       // Wrap handler to convert JSON-as-text into proper JSON content items
//       const wrappedHandler = async (args: any) => {
//         const result = await handler(args);
//         try {
//           const content = result?.content;
//           if (Array.isArray(content)) {
//             result.content = content.map((item: any) => {
//               if (item?.type === "text" && typeof item.text === "string") {
//                 const s = item.text.trim();
//                 // Basic JSON detection: starts/ends with {...} or [...]
//                 const looksJson =
...
//                   } catch {
//                     // fall through; keep original text
//                   }
//                 }
//               }
//               return item;
...
//           }
//         } catch {
//           // keep original result if anything goes wrong
//         }
//         return result;
//       };

//       return orig(name, fixedDef, wrappedHandler);
//     };
//   }

//   // Call the patch immediately after creating the server
